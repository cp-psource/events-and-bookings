...
class Eab_Events_EventCancel {
	...
	function __construct () {
		...
		$this->_default_subject = __('Schlechte Nachrichten! EVENT_NAME wurde abgebrochen', Eab_EventsHub::TEXT_DOMAIN);
		$this->_default_message = __("Hallo, USER_NAME\n\nUnfortunately, Leider wurde unser EVENT_LINK-Event abgesagt.", Eab_EventsHub::TEXT_DOMAIN);
	}
...
EOPublicCancellationCss;
	...
	function inject_cancelled_status ($options, $event) {
		...
		$title = __('Abgebrochen', Eab_EventsHub::TEXT_DOMAIN);
	...
	function inject_cancel_button ($after, $event) {
		...
		$button = '' .
			'<div class="misc-eab-section">' .
				'<input type="button" class="button" id="eab-event-cancel_event" value="' . esc_attr(__('Ereignis abbrechen', Eab_EventsHub::TEXT_DOMAIN)) . '" />' .
			'</div>' .
		'';
	...
	function _get_cancelled_notice ($event) {
		return '<div class="psourceevents-buttons eab-event_cancelled">' .
			'<p>' . __('Veranstaltung wurde abgesagt', Eab_EventsHub::TEXT_DOMAIN) . '</p>' .
		'</div>';
	}
...
<div id="eab-settings-eab_cancelations" class="eab-metabox postbox">
	<h3 class="eab-hndle"><?php _e('Einstellungen Ereignisstornierung', Eab_EventsHub::TEXT_DOMAIN); ?></h3>
	<div class="eab-inside">
		<div class="eab-settings-settings_item">
			<label for="eab_cancellations-hide_events"><?php _e('Abgebrochene Ereignisse ausblenden', Eab_EventsHub::TEXT_DOMAIN); ?></label>
		...
		<div class="eab-settings-settings_item">
			<label for="eab_cancellations-email_batch_limit"><?php _e('E-Mail-Batch-Limit', Eab_EventsHub::TEXT_DOMAIN); ?>:</label>
			<span><?php echo $tips->add_tip(__('Dies ist die maximale Anzahl von E-Mails, die auf einmal gesendet werden. Der Rest wird für den Versand geplant.', Eab_EventsHub::TEXT_DOMAIN)); ?></span>
		...
		<div class="eab-note">
			<?php _e('Dies ist die E-Mail, die bei Absage der Veranstaltung an Ihre Teilnehmer gesendet wird.', Eab_EventsHub::TEXT_DOMAIN); ?>
		</div>
		<div class="eab-settings-settings_item">
	    	<label for="eab_event-eab_cancelations-from"><?php _e('Von der Email Adresse', Eab_EventsHub::TEXT_DOMAIN); ?></label>
			<span><?php echo $tips->add_tip(__('Dies ist die Adresse, von der aus die Stornierungs-E-Mail gesendet wird', Eab_EventsHub::TEXT_DOMAIN)); ?></span>
			<input type="text" id="eab_event-eab_cancelations-from" name="eab_cancelations[email-from]" value="<?php esc_attr_e($from); ?>" />
	    </div>
	    <div class="eab-settings-settings_item">
	    	<label for="eab_event-eab_cancelations-subject"><?php _e('Email Betreff', Eab_EventsHub::TEXT_DOMAIN); ?></label>
			<span><?php echo $tips->add_tip(sprintf(__('Dies ist der E-Mail-Betreff. Du kannst diese Makros verwenden: <code>%s</code>', Eab_EventsHub::TEXT_DOMAIN), $macros)); ?></span>
	    ...
	    <div class="eab-settings-settings_item">
	    	<label for="eab_event-eab_cancelations-body"><?php _e('Nachrichtentext', Eab_EventsHub::TEXT_DOMAIN); ?></label>
			<span><?php echo $tips->add_tip(sprintf(__('Dies ist der E-Mail-Text. Du kannst diese Makros verwenden: <code>%s</code>', Eab_EventsHub::TEXT_DOMAIN), $macros)); ?></span>
			<?php wp_editor($body, 'eab_cancelations-email-body', array(
				'name' => 'eab_cancelations-email-body',
			)); ?>
	    </div>
	    <div class="eab-settings-settings_item"><small><?php printf(__('Du kannst diese Makros in Deinem Betreff und Nachrichtentext verwenden: <code>%s</code>', Eab_EventsHub::TEXT_DOMAIN), $macros) ?></small></div>
	<?php if ($events) { ?>
	    <div class="eab-settings-settings_item">
	    	<input type="button" class="button" id="eab_event-eab_cancelations-preview" value="<?php esc_attr_e(__('VORSCHAU', Eab_EventsHub::TEXT_DOMAIN)); ?>" />
	    	<?php _e('Verwende diese Ereignisdaten:', Eab_EventsHub::TEXT_DOMAIN); ?>
...
$(function () {
	...
	$("#eab_event-eab_cancelations-preview").on("click", function () {
		...
		$container.html('<?php echo esc_js(__("Bitte hör auf... ", Eab_EventsHub::TEXT_DOMAIN)); ?>');
		$.post(ajaxurl, {
