...
class Eab_Upcoming_Widget extends Eab_Widget {
    ...
    function __construct () {
    	$this->_defaults = apply_filters('eab-widgets-upcoming-default_fields', array( 
			'title' 				=> __('PSE-Kommende Veranstaltungen', $this->translation_domain),
			'excerpt' 				=> false,
		...
		$widget_ops = array(
			'description' => __('Liste der bevorstehenden Ereignisse anzeigen', $this->translation_domain),
		);
        $control_ops = array(
        	'title' => __('Bevorstehend', $this->translation_domain),
        );
		parent::__construct('psource_event_upcoming', __('Kommende Veranstaltungen', $this->translation_domain), $widget_ops, $control_ops);
    }
    ...
    function widget ($args, $instance) {
		...
		$title = apply_filters('widget_title', ( empty($options['title']) ) ? __('Bevorstehend', $this->translation_domain) : $options['title'], $options, $this->id_base);
		$query_args = array(
		...
		if ($options['lookahead'] && is_numeric($options['lookahead'])) {
			...
			$lookahead_func = function() {return ' . $options[lookahead] . ';};
		...
		if (is_array($_events) && count($_events) > 0) {
			...
			foreach ($_events as $_event) {
				...
				echo '<a href="' . get_permalink($_event->get_id()) . '" class="' . ($post instanceof WP_Post && $_event->get_id() == $post->ID ? 'current' : 'not_current') . '" >' .
					...
					$_event->get_title() .
				'</a>';
				if (!empty($options['dates'])) echo '<div class="psourceevents-date">' . Eab_Template::get_event_dates($_event) . '</div>';
	...
	<div style="text-align:left">
            ...
            <label for="<?php echo $this->get_field_id('dates'); ?>" style="display:block;">
				...
				/>
            	<?php _e('Datum anzeigen', $this->translation_domain); ?>
            </label>
            <label for="<?php echo $this->get_field_id('excerpt'); ?>" style="display:block;">
             ...
             <label for="<?php echo $this->get_field_id('excerpt_words_limit'); ?>" style="display:block; margin-left:1.8em">
            	<?php _e('Beschränke meinen Auszug auf so viele Wörter <small>(<code>0</code> ohne Begrenzung)</small>:', $this->translation_domain); ?>
				<input type="text" 
					size="2"
            ...
            <label for="<?php echo $this->get_field_id('thumbnail'); ?>" style="display:block;">
				...
				/>
            	<?php _e('Miniaturbild anzeigen', $this->translation_domain); ?>
           </label>
           ...
           <label for="<?php echo $this->get_field_id('category'); ?>" style="line-height:35px;display:block;">
            	<?php _e('Nur Ereignisse aus dieser Kategorie', $this->translation_domain); ?>:
				<select id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>">
					<option><?php _e('Alle', $this->translation_domain);?></option>
