...
jQuery(function($) {
    ...
    jQuery("#incsub_event-accept_payments").on('change', function () {
		if (!jQuery("#incsub_event-accept_payments").is(":checked")) {
			jQuery("#eab-settings-paypal").hide();
		} else {
    ...
    jQuery("#incsub_event_paid").on('change', function () {
		if (jQuery("#incsub_event_paid").val() == 0) {
			jQuery(".incsub_event-fee_row").hide();
		} else {
...
$(function () {
	// Init API toggle
	$("#incsub_event-accept_api_logins").on('change', toggle_api_settings);
	toggle_api_settings();

	// Init Appearance toggle
	$("#incsub_event-override_appearance_defaults").on('change', toggle_appearance_settings);
	...
	box_root.append($submit);
        ...
        if( hash ) {
			...
			if (obj.length) {
				...
				root.find("h3:first").trigger('click');
			}
		} else {
			root.find("h3:first").trigger('click');
        }
