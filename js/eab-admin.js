jQuery(function($) {
    ...
    jQuery(".psource_event_picker").datepicker({
        "dateFormat": "yy-mm-dd",
        "changeMonth": true,
        "changeYear": true,
    ...
    jQuery('body').on('focus', '.psource_event_time_picker', function() {
        $(this).timepicker({
            showNowButton: true,
    ...
    jQuery("#eab-add-more-button").click(function() {
        ...
        jQuery("#psource_event_start_" + row_id + " , #psource_event_end_" + row_id).datepicker({
            "dateFormat": "yy-mm-dd",
            "changeMonth": true,
            "changeYear": true,
            "firstDay": parseInt(eab_event_localized.start_of_week, 10) ? parseInt(eab_event_localized.start_of_week, 10) : 0
        ...
        jQuery("#psource_event_start_time_" + row_id + " , #psource_event_end_time_" + row_id).timepicker({
            showNowButton: true,
            showDeselectButton: true,
            showCloseButton: true
        });
    ...
    jQuery("#psource_event_paid").change(function() {
        if (jQuery("#psource_event_paid").val() == 0) {
            ...
            jQuery(".psource_event-fee_row").show();
        }
    });
    ...
    if (!jQuery('#psource-event-bookings').hasClass('closed')) {
        jQuery('#psource-event-bookings').addClass('closed');
    }
    jQuery('#psource-event-bookings .hndle').append('<span class="eab-expand-metabox">' + eab_event_localized["view_all_bookings"] + '</span>');
    if (!jQuery('#psource-event-wizard').hasClass('closed')) {
        jQuery('#psource-event-wizard').addClass('closed');
    }
    jQuery('#psource-event-wizard .hndle').append('<a href="edit.php?post_type=psource_event&page=eab_welcome" class="eab-expand-metabox">' + eab_event_localized["back_to_gettting_started"] + '</a>');

    if (jQuery('.eab-section-block').length == 2) {
        jQuery('.eab-section-heading').hide();
    }

    jQuery("#psource-event").on("change", 'input.psource_event', function() {
        var _c = jQuery(this).attr('id').replace(/psource_event_[a-z_]+_/gi, '');
        _eab_validate_when(_c);
    });

    jQuery("#psource-event").on("change", ".psource_event_no_start_time", function() {
        var $me = jQuery(this),
            _c = $me.attr('id').replace(/psource_event_[a-z_]+_/gi, '');
        if ($me.is(":checked")) jQuery("#psource_event_start_time_" + _c).hide();
        else jQuery("#psource_event_start_time_" + _c).show();
    });
    jQuery(".psource_event_no_start_time").each(function() { jQuery(this).trigger("change"); });

    jQuery("#psource-event").on("change", ".psource_event_no_end_time", function() {
        var $me = jQuery(this),
            _c = $me.attr('id').replace(/psource_event_[a-z_]+_/gi, '');
        if ($me.is(":checked")) jQuery("#psource_event_end_time_" + _c).hide();
        else jQuery("#psource_event_end_time_" + _c).show();
    });
    jQuery(".psource_event_no_end_time").each(function() { jQuery(this).trigger("change"); });

    jQuery("#psource-event").on("change", ".psource_event_start", function() {
        var $me = jQuery(this),
            $end = $me.parents(".eab-section-block").find(".psource_event_end"),
            _c = $me.attr('id').replace(/psource_event_[a-z_]+_/gi, ''),
            _start, _end, _tmp;
    ...
    function _eab_get_stat_end_datetime(_c) {
        var _start, _end;
        _start = false;
        if (jQuery('#psource_event_start_' + _c).val() != '') {
            _start = new Date(jQuery('#psource_event_start_' + _c).val());
            if (jQuery('#psource_event_start_time_' + _c).val() != '' || jQuery('#psource_event_no_start_time_' + _c).is(":checked")) {
                var _start_time = (jQuery('#psource_event_no_start_time_' + _c).is(":checked") ? '00:01' : jQuery('#psource_event_start_time_' + _c).val()),
        ...
        _end = false;
        if (jQuery('#psource_event_end_' + _c).length && jQuery('#psource_event_end_' + _c).val() != '') {
            _end = new Date(jQuery('#psource_event_end_' + _c).val());
            if (jQuery('#psource_event_end_time_' + _c).val() != '' || jQuery('#psource_event_no_end_time_' + _c).is(":checked")) {
                var _end_time = (jQuery('#psource_event_no_end_time_' + _c).is(":checked") ? '23:59' : jQuery('#psource_event_end_time_' + _c).val()),
    ...
    function _eab_validate_when(_c) {
        ...
        if ((!_start || !_end) || _start.getTime() >= _end.getTime()) {
            jQuery('#psource_event_start_' + _c).addClass('error');
            jQuery('#psource_event_start_time_' + _c).addClass('error');
            jQuery('#psource_event_end_' + _c).addClass('error');
            jQuery('#psource_event_end_time_' + _c).addClass('error');
            jQuery('input.button-primary.button-large').attr('disabled', true);
            return false;
        } else {
            jQuery('#psource_event_start_' + _c).removeClass('error');
            jQuery('#psource_event_start_time_' + _c).removeClass('error');
            jQuery('#psource_event_end_' + _c).removeClass('error');
            jQuery('#psource_event_end_time_' + _c).removeClass('error');
    ...
    (function($) {
        ...
        function toggle_api_settings() {
            if ($("#psource_event-accept_api_logins").is(":checked")) $("#eab-settings-apis .eab-inside").show();
            else $("#eab-settings-apis .eab-inside").hide();
        }

        // Appearance toggling
        function toggle_appearance_settings() {
            if ($("#psource_event-override_appearance_defaults").is(":checked")) {
                $("#psource_event-archive_template").attr("disabled", false);
                $("#psource_event-single_template").attr("disabled", false);
            } else {
                $("#psource_event-archive_template").attr("disabled", true);
                $("#psource_event-single_template").attr("disabled", true);
            }
        }
        ...
        $(function() {
            ...
            var $times = $("#psource-event input.psource_event");
            $times.each(function() {
                var _c = $(this).attr('id').replace(/psource_event_[a-z_]+_/gi, '');
                _eab_validate_when(_c);
            });
